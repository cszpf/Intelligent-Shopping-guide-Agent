...
import json
import re
from static_data_computer import necessaryTag, labelToTag, ask_slot, listInfo, adjustableSlot, \
...
def get_change_intent(domain, sentence):
    ...
    target_to_label = {'价格': 'price', '硬盘': 'disk', '内存': 'memory'}
    pos_word = ['贵', '高', '大', '好']
...
class Computer_Dialogue():
    ...
    def fill_message(self, tag):
        ...
        op_dict = {'l': '>=', 'm': '=', 'u': '<='}
        bi_tag = ['brand', 'experience', 'function', 'cpu', 'gpu']
        for t in tag:
            op = '='
            name = t['type']
            if t['type'] in bi_tag:
