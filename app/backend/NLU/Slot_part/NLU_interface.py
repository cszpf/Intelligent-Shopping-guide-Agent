...
def get_camera_slot_dl(text):
    ...
    FLAGS.config_file = CONFIG.NLU_path + '\\Slot_part\\forum_config\\config_file'
    FLAGS.log_file = CONFIG.NLU_path + '\\Slot_part\\forum_config\\log\\train.log'
    FLAGS.ckpt_path = CONFIG.NLU_path + '\\Slot_part\\forum_ckpt'
    FLAGS.map_file = CONFIG.NLU_path + '\\Slot_part\\forum_config\\maps.pkl'
...
def get_phone_slot_dl(text):
    ...
    """
    tf.reset_default_graph()   # FIXME: 为了可以保证重复多次输入(不过会很慢)整合代码时，可以尝试将NLU和DM分离，使用统一的文件管理。
    FLAGS.config_file = CONFIG.NLU_path + '\\Slot_part\\forum_config\\phone_config_file'
    FLAGS.log_file = CONFIG.NLU_path + '\\Slot_part\\forum_config\\log\\phone_train.log'
    FLAGS.ckpt_path = CONFIG.NLU_path + '\\Slot_part\\forum_ckpt\\phone'
    FLAGS.map_file = CONFIG.NLU_path + '\\Slot_part\\forum_config\\phone_maps.pkl'
